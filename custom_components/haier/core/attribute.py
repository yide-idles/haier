...
class V1SpecAttributeParser(HaierAttributeParser, ABC):
    def parse_attribute(self, attribute: dict) -> HaierAttribute:
        ...
        if (
            attribute["writable"]
            and equals_ignore_case(attribute["valueRange"]["type"], "STEP")
            and contains_any_ignore_case(
                attribute["valueRange"]["dataStep"]["dataType"], ["Integer", "Double"]
            )
        ):
            return self._parse_as_number(attribute)

        # 一定要在select之前，不然会被select覆盖
        if attribute["writable"] and V1SpecAttributeParser._is_binary_attribute(
            attribute
        ):
            return self._parse_as_switch(attribute)

        if attribute["writable"] and equals_ignore_case(
            attribute["valueRange"]["type"], "LIST"
        ):
    ...
    def parse_global(self, attributes: List[dict]):
        ...
        # 燃气热水器
        if (
            "outWaterTemp" in all_attribute_keys
            and "targetTemp" in all_attribute_keys
            and "totalUseGasL" in all_attribute_keys
        ):
    ...
    def _parse_as_sensor(attribute):
        if V1SpecAttributeParser._is_binary_attribute(attribute):
            return HaierAttribute(
                attribute["name"], attribute["desc"], Platform.BINARY_SENSOR
            )
        ...
        if equals_ignore_case(attribute["valueRange"]["type"], "STEP"):
            state_class, device_class, unit = (
                V1SpecAttributeParser._guess_state_class_device_class_and_unit(
                    attribute
                )
            )
            if device_class:
        ...
        return HaierAttribute(
            attribute["name"], attribute["desc"], Platform.SENSOR, options, ext
        )
    ...
    def _parse_as_number(attribute):
        ...
        _, _, unit = V1SpecAttributeParser._guess_state_class_device_class_and_unit(
            attribute
        )
        if unit:
            options["native_unit_of_measurement"] = unit
        ...
        return HaierAttribute(
            attribute["name"], attribute["desc"], Platform.NUMBER, options
        )
    ...
    def _parse_as_select(attribute):
        ...
        return HaierAttribute(
            attribute["name"], attribute["desc"], Platform.SELECT, options, ext
        )
    ...
    def _parse_as_switch(attribute):
        ...
        return HaierAttribute(
            attribute["name"], attribute["desc"], Platform.SWITCH, options
        )
    ...
    def _parse_as_climate(attributes: List[dict], feature_fields: List[str]):
        ...
        options = {
            "min_temp": float(
                target_temperature_attr["valueRange"]["dataStep"]["minValue"]
            )
            + 16,
            "max_temp": float(
                target_temperature_attr["valueRange"]["dataStep"]["maxValue"]
            )
            + 16,
            "target_temperature_step": float(
                target_temperature_attr["valueRange"]["dataStep"]["step"]
            ),
        }
    ...
    def _parse_as_gas_water_heater(attributes: List[dict]):
        ...
        options = {
            ...
            "target_temperature_step": target_temperature_attr["valueRange"][
                "dataStep"
            ]["step"],
        }
        ...
        return HaierAttribute(
            "gas_water_heater", "GasWaterHeater", Platform.WATER_HEATER, options, ext
        )
    ...
    def _is_binary_attribute(attribute):
        ...
        return (
            equals_ignore_case(valueRange["type"], "LIST")
            and len(valueRange["dataList"]) == 2
            and contains_any_ignore_case(
                valueRange["dataList"][0]["data"], ["true", "false"]
            )
            and contains_any_ignore_case(
                valueRange["dataList"][1]["data"], ["true", "false"]
            )
        )
